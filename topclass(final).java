...
public class TopClass implements ActionListener, KeyListener {
	...
	private static final int PIPE_GAP = SCREEN_HEIGHT / 3; // distance in pixels between pipes
	private static final int PIPE_WIDTH = SCREEN_WIDTH / 8, PIPE_HEIGHT = 4 * PIPE_WIDTH;
	private static int BIRD_WIDTH=60, BIRD_HEIGHT=40 ;
	...
	private static final int BIRD_JUMP_DIFF = 2, BIRD_FALL_DIFF = 2,
			BIRD_JUMP_HEIGHT = PIPE_GAP - 2*BIRD_HEIGHT - BIRD_JUMP_DIFF * 4;
	...
	private void buildFrame() {
		Image icon = Toolkit.getDefaultToolkit().getImage(this.getClass().getResource("resources/dd7.gif"));
		//Image icon = Util.loadImage("resources/blue_bird.png");
		//BIRD_WIDTH=icon.getWidth(null);
		//BIRD_HEIGHT=icon.getHeight(null);
...
/*		else if (e.getKeyCode() == KeyEvent.VK_R ) // game has ended
		 {
			//play("sfx_swooshing");
			//birdYTracker = SCREEN_HEIGHT / 2 - BIRD_HEIGHT; // need to reset the bird's starting height
		//	loopVar=false;
...
//			play("cancel");
	...
	private void collisionDetection(BottomPipe bp1, BottomPipe bp2, TopPipe tp1, TopPipe tp2, Bird bird) {
		...
		if (bird.getY() + BIRD_HEIGHT > SCREEN_HEIGHT * 7 / 8) { // ground
			...
			//restart();
			//JOptionPane.showMessageDialog(f,"Game Over"+"  Best Score : " + s);
			...
			for (int i = firstI; i < r.getWidth() + firstI; i++) { //
				for (int j = firstJ; j < r.getHeight() + firstJ; j++) {
					if ((b1.getRGB(i, j) & 0xFF000000) != 0x00
						...
						pgs.sendText("Game Over"+"  Best Score : " + s );
						
						/*int ll=JOptionPane.showConfirmDialog(f, "Game Over"+"  Best Score : " + s,null,JOptionPane.YES_NO_OPTION);
						if(ll==JOptionPane.YES_OPTION)
						{
							...
							//t.start();
							//tc.gameScreen(false);
							//loopVar = true; // stop the game loop
							//gamePlay = true; // game has ended
						...
						//tc.gameScreen(true);
						/*nnnnnn*///pgs = new PlayGameScreen(SCREEN_WIDTH, SCREEN_HEIGHT, true);
						...
						//JOptionPane.showMessageDialog(f,"Game Over"+"  Best Score : " + s);
                        //int ll=JOptionPane.showConfirmDialog(f, "Game Over"+"  Best Score : " + s,null,JOptionPane.OK_OPTION);
